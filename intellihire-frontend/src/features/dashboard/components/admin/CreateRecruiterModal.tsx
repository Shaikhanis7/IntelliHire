    ...
    >
      <div style={{ width:'100%', maxWidth:460, background:C.bgCard, borderRadius:20, border:`1px solid ${C.borderMid}`, boxShadow:'0 24px 64px rgba(15,30,80,0.18)', animation:'dbSlideUp 0.22s ease both', overflow:'hidden' }}>
        ...
        <div style={{ padding:'20px 24px 16px', borderBottom:`1px solid ${C.border}`, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          ...
          <button onClick={onClose} style={{ width:30, height:30, borderRadius:8, border:`1px solid ${C.border}`, background:C.bgCard, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', transition:'all 0.15s' }}
            ...
            <>
              ...
              <div style={{ display:'flex', gap:10 }}>
                ...
                <button onClick={handleSubmit}  disabled={busy} style={{ ...primaryBtn, flex:2, opacity:busy ? 0.75 : 1, display:'flex', alignItems:'center', justifyContent:'center', gap:7 }}>
                  {busy ? <><Loader2 size={14} style={{ animation:'dbSpin 1s linear infinite' }} /> Creating…</> : <><UserPlus size={14} /> Create Recruiter</>}
                </button>
              </div>
            </>
          )}
  ...
  return (
    <>
      <div style={{ position:'fixed', right:28, bottom:28, zIndex:900 }}>
        {label && (
          <div style={{ position:'absolute', right:62, top:'50%', transform:'translateY(-50%)', background:C.text, color:C.white, fontSize:12, fontWeight:600, padding:'5px 11px', borderRadius:7, whiteSpace:'nowrap', pointerEvents:'none', animation:'dbFadeIn 0.15s ease', fontFamily:F.body }}>
            Create Recruiter
            <span style={{ position:'absolute', right:-5, top:'50%', transform:'translateY(-50%)', width:0, height:0, borderTop:'5px solid transparent', borderBottom:'5px solid transparent', borderLeft:`5px solid ${C.text}` }} />
          </div>
        )}
        <button
          ...
          style={{ width:52, height:52, borderRadius:'50%', background:C.gradBlue, border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', boxShadow:C.shadowBlue, transition:'transform 0.18s, box-shadow 0.18s' }}
          onMouseDown={e => { (e.currentTarget as HTMLButtonElement).style.transform = 'scale(0.93)'; }}
          onMouseUp={e   => { (e.currentTarget as HTMLButtonElement).style.transform = 'scale(1)'; }}
        >
          <UserPlus size={20} color={C.white} />
        </button>
      </div>
