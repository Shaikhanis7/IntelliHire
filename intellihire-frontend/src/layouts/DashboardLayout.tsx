...
import {
  ...
  ChevronRight, Bell, Search, Zap, ListChecks, ShieldCheck, UserPlus,
} from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { CreateRecruiterFAB } from '../features/admin/components/CreateRecruiterFAB';

/* ─── Tokens — exact match with AuthPage + LandingPage ──────────────────────
   AuthPage outer bg:  linear-gradient(135deg, #eef2fb, #f0f4ff, #ebf0fa, #edf1fb)
   AuthPage C.navy:    #1e3a8a
   AuthPage C.blue:    #2563eb
   AuthPage C.gradBlue: linear-gradient(135deg, #1e3a8a 0%, #2563eb 55%, #3b82f6 100%)
   AuthPage C.gradPanel: linear-gradient(155deg, #1e3a8a, #1d4ed8, #2563eb)
   AuthPage C.border:  rgba(30,58,138,0.12)
   AuthPage C.surface: #ffffff
   AuthPage C.bg:      #f5f6fa
────────────────────────────────────────────────────────────────────────────── */
const S = {
  /* Sidebar — mirrors AuthPage outer wrapper gradient */
...
/* ─── Sidebar decorative bg elements ─────────────────────────────────────────── */
const SidebarDeco: React.FC = () => (
  <>
    {/* Top accent line — same gradient tone as AuthPage panel */}
    <div style={{
      position: 'absolute', top: 0, left: 0, right: 0, height: 2,
      background: 'linear-gradient(90deg, transparent, rgba(37,99,235,0.5) 35%, rgba(59,130,246,0.55) 65%, transparent)',
      zIndex: 1,
    }} />
    {/* Blue bloom — top right, matches AuthPage drift blob */}
    <div style={{
    ...
    <div style={{
      position: 'absolute', bottom: 100, left: -50, width: 200, height: 200, borderRadius: '50%',
    ...
    {/* Right border — matches AuthPage card left-border tone */}
    <div style={{
      position: 'absolute', top: 0, right: 0, bottom: 0, width: 1,
      background: 'linear-gradient(180deg, rgba(37,99,235,0.22) 0%, rgba(37,99,235,0.08) 55%, rgba(99,102,241,0.14) 100%)',
    }} />
  </>
);
  ...
  return (
    <div style={{
      ...
      <div
        ...
        style={{
          position: 'fixed', inset: '0 auto 0 0', width: 248,
          background: S.sidebarBg,
          display: 'flex', flexDirection: 'column',
          zIndex: 40, overflow: 'hidden',
          transition: 'transform 0.3s cubic-bezier(0.77,0,0.18,1)',
          boxShadow: S.shadowPanel,
        }}
      >
        <SidebarDeco />
        ...
        >
          ...
          <div>
            ...
            <span style={{ fontSize: 10, color: S.textFaint, fontWeight: 500, letterSpacing: 0.3 }}>
        ...
        >
          <span style={{
            ...
            background: roleChip.dim, color: roleChip.color,
            border: `1px solid ${roleChip.border}`,
            fontSize: 10, fontWeight: 800, letterSpacing: 0.6, textTransform: 'uppercase',
          }}>
            <span style={{
              width: 5, height: 5, borderRadius: '50%',
              background: roleChip.color, flexShrink: 0,
              boxShadow: `0 0 6px ${roleChip.color}55`,
            }} />
            {roleChip.label}
          </span>
        </motion.div>
        ...
        {isAdmin && (
          ...
          >
            <SLabel label="Admin" />
            <div style={{
              borderRadius: 11, background: S.redDim,
              border: `1px solid ${S.redBorder}`, padding: '10px 13px',
              display: 'flex', alignItems: 'center', gap: 10,
            }}>
              <div style={{
                width: 30, height: 30, borderRadius: 8,
                background: 'rgba(220,38,38,0.06)', border: `1px solid ${S.redBorder}`,
            ...
            <div style={{
              ...
              <span style={{ position: 'relative', display: 'inline-flex', flexShrink: 0 }}>
                <span style={{
                  ...
                  background: S.blue, top: -2, left: -2, opacity: 0.45,
                  animation: 'ping3 1.7s ease-out infinite',
                }} />
        ...
        <motion.div
          ...
          style={{
            margin: '0 10px 16px', padding: '11px 12px', borderRadius: 12,
            background: 'rgba(37,99,235,0.05)',
        ...
        >
          <div style={{
            ...
            background: roleChip.grad,
            fontSize: 13, fontWeight: 800, color: '#fff',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            boxShadow: `0 2px 10px ${roleChip.color}28`,
          }}>
            {user?.name?.[0]?.toUpperCase() ?? 'U'}
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ fontSize: 12.5, fontWeight: 700, color: S.textHi, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', margin: 0 }}>
              {user?.name}
            </p>
            <p style={{ fontSize: 10.5, color: S.textLow, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', margin: 0 }}>
              {user?.email ?? user?.role ?? 'User'}
            </p>
          </div>
          <motion.button
            onClick={logout} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} title="Sign out"
            ...
            onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.color = S.red; (e.currentTarget as HTMLButtonElement).style.background = S.redDim; }}
            onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.color = S.textFaint; (e.currentTarget as HTMLButtonElement).style.background = 'transparent'; }}
          >
            <LogOut size={14} />
          </motion.button>
        </motion.div>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            onClick={() => setMobileOpen(false)}
            style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.35)', zIndex: 39, backdropFilter: 'blur(6px)' }}
          />
        )}
      </AnimatePresence>

      {/* ══ MAIN AREA ══ */}
      <div className="main-col" style={{ marginLeft: 248, flex: 1, display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
        ...
        <motion.header
          ...
          style={{
            ...
            /* same glassmorphism as AuthPage sliding panel region */
        ...
        >
          <button className="mob-btn" onClick={() => setMobileOpen(true)}
            style={{ background: 'none', border: 'none', cursor: 'pointer', color: M.blue, padding: 4, display: 'flex' }}>
            <Menu size={21} />
          </button>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            {isAdmin && (
              <span style={{
                ...
                background: 'rgba(220,38,38,0.07)', color: S.red, border: `1px solid rgba(220,38,38,0.22)`,
                display: 'inline-flex', alignItems: 'center', gap: 4,
              }}>
                <ShieldCheck size={9} /> Admin
              </span>
            )}
            {/* Page title — Fraunces matches AuthPage panel heading font */}
            <span style={{
              fontSize: 15, fontWeight: 800, color: M.text,
              letterSpacing: '-0.4px', fontFamily: "'Fraunces', serif",
            }}>
              {activePage}
            </span>
            {activePage === 'Sourcing' && (
              <motion.span
                ...
                style={{
                  ...
                  background: M.gradBlue, color: '#fff',
          ...
          <motion.div
            ...
            style={{
              ...
              transition: 'all 0.2s', boxShadow: '0 1px 3px rgba(15,23,42,0.05)',
            }}
          >
            ...
            <kbd style={{
              ...
              background: M.blueDim, border: `1px solid ${M.border}`,
              fontSize: 10, color: M.textFaint, fontFamily: 'inherit',
            }}>⌘K</kbd>
          </motion.div>

          {/* Bell */}
          <motion.button
            whileHover={{ scale: 1.05, borderColor: M.borderHov }}
            whileTap={{ scale: 0.95 }}
            style={{
              position: 'relative', width: 36, height: 36, borderRadius: 10,
              background: M.surface, border: `1px solid ${M.border}`,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              cursor: 'pointer', color: M.textFaint,
              boxShadow: '0 1px 3px rgba(15,23,42,0.05)', transition: 'all 0.2s',
            }}
          ...
          >
            ...
            <span style={{
              ...
              borderRadius: '50%', background: M.gradBlue, border: `1.5px solid ${M.bg}`,
            }} />
          </motion.button>
        </motion.header>

        <main id="main-scroll" style={{ flex: 1, overflowY: 'auto', padding: '26px 26px 44px', animation: 'pageIn 0.38s ease both' }}>
          <Outlet />
        </main>
      </div>
