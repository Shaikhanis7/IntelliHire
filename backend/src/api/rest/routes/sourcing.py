"""
src/api/rest/routes/sourcing_router.py
"""
...
async def sourcing_history(
    ...
    return {"job_id": job_id, "sourced": len(runs) > 0, "runs": runs}


# ── GET /sourcing/{sourcing_id}/candidates ────────────────────────────────────
...
async def sourcing_candidates(
    ...
    if job_id is not None:
        ...
        if not result.scalar_one_or_none():
            raise HTTPException(404, f"Sourcing run #{sourcing_id} not found for job_id={job_id}.")
...
async def apply_sourced_candidate(
    ...
    try:
        result = await apply_for_job_with_resume(
            db, candidate_id=data.candidate_id, job_id=data.job_id, file=None,
        )
...
async def sourcing_runs_for_job(
    ...
    result = await db.execute(
        select(Sourcing).where(Sourcing.job_id == job_id).order_by(Sourcing.id.desc())
    )
...
async def shortlist_sourced_candidate(
    ...
    # 2. Update ALL SourcingCandidate rows for this candidate
    await db.execute(
