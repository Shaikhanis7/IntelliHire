...
# Kicks off Celery task. Returns sourcing_id immediately.
...
# Returns [] if the Celery task hasn't finished writing candidates yet.
...
async def sourcing_runs_for_job(
    ...
    {
        "job_id": 42,
        "sourced": true,          // false = no runs at all
        "total_runs": 3,
        "runs": [
            {
                "sourcing_id": 7,
...
#     # Fetch the sourcing candidate row to get scores + rank
#     sc_result = await db.execute(
#         select(SourcingCandidate).where(
#             SourcingCandidate.candidate_id == data.candidate_id,
#         )
#         .order_by(SourcingCandidate.id.desc())   # latest sourcing run first
#         .limit(1)
#     )
...
#         )
#         db.add(app)
#         await db.flush()   # get app.id without full commit

    # # Write sourcing scores + rank onto the application row
    ...
    #     app.source_tag     = sourcing_candidate.source_tag
...
async def source(
    ...
    source_candidates_task.delay(
        ...
        data.count,
        data.mode,
        sourcing.id,   # ← pass pre-created ID so Celery doesn't create a second one
    )
...
async def shortlist_sourced_candidate(
    ...
    # 2. ← KEY FIX: update ALL SourcingCandidate rows for this candidate
    #    so every sourcing run's data shows "applied_shortlisted" on re-fetch
    await db.execute(
