"""
src/api/rest/routes/application_router.py
"""
...
async def apply(
    ...
    user: dict = Depends(require_candidate),          # ← guards candidate only
):
    candidate = await get_candidate_by_user_id(db, user["id"])
    if not candidate:
        raise HTTPException(404, "Candidate profile not found.")
    return await apply_for_job_with_resume(
        db, candidate_id=candidate.id, job_id=job_id,
...
# Recruiter (or admin) views all applicants for a job (ranked)
...
# Recruiter (or admin) triggers shortlisting — scores all pending, returns top N
